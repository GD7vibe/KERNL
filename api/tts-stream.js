    ...
    ws.on('message', async (data) => {
      try {
        ...
        if (event.type === 'audio.delta' && event.delta) {
          ...
          if (browserConnected) {
            try {
              // Send base64 chunk as SSE event — frontend decodes and feeds to AudioContext
        ...
        if (event.type === 'audio.done') {
          ...
          ws.close();
          // Save to Supabase after streaming so future plays are instant
          if (allChunks.length > 0) {
            await saveAudioToStorage(filename, Buffer.concat(allChunks));
          }
          resolve();
        }
