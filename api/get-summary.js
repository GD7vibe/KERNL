...
module.exports = async (req, res) => {
  ...
  try {
    if (author) {
      const keyNoSpoilers = title + '||' + author + '||nospoilers';
      const keySpoilers = title + '||' + author + '||spoilers';
      const { data: exactRows } = await supabase
        .from('summaries')
        .select('title, author, html, plain, words, genre, lookup_key')
        .in('lookup_key', [keyNoSpoilers, keySpoilers])
        .limit(1);
      if (exactRows && exactRows.length > 0) return res.status(200).json(sanitise(exactRows[0]));
    }
    ...
    const distinctWords = normTitle.split(' ').filter(w => w.length > 4);
